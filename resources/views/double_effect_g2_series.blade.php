	...
	<div class="page-header">
		...
		<div class="page-header-breadcrumb">
			<ul class="breadcrumb-title">
				...
				<li class=""><a href="#!">Double Effect G2 Series</a>
				</li>
			</ul>
		</div>
	</div>
							...
							<div class="">
								<div class="row">
									...
									<div class="col-lg-4">
										...
										<span class="messages emsg hidden" id="calorific_value_error"><p class="text-danger error">Please Enter a Valid Hot Water In</p></span>
									</div>
									<div class="col-lg-2">
										<label>({{ $units_data[$unit_set->TemperatureUnit] }})</label>
									</div>
...
$('input:radio[name="fuel_type"], input:radio[name="fuel_value_type"]').on('change', function() {

		model_values.fuel_value_type = $(".fuel_value_type:checked").val();
		model_values.fuel_type = $(".fuel_type:checked").val();
		//alert(model_values.fuel_value_type);
	if (model_values.fuel_value_type == 'NaturalGas') {

		model_values.fuel_cv_std = 9650;

		if(model_values.fuel_type =='Gross')
		{
			model_values.calorific_value = 9650; 
			$(".fuel_cv_stds").show();
			$("#fuel_cv_std").html(model_values.fuel_cv_std);
		}
		if(model_values.fuel_type =='Normal')
		{
			$(".fuel_cv_stds").hide();
			model_values.calorific_value = 9000; 
			
		}
		
	} else {
		
		model_values.fuel_cv_std = 10960;
  		if(model_values.fuel_type =='Gross')
		{
			model_values.calorific_value = 10960; 
			$(".fuel_cv_stds").show();
			$("#fuel_cv_std").html(model_values.fuel_cv_std);
		}
		if(model_values.fuel_type =='Normal')
		{
			$(".fuel_cv_stds").hide(); 
			model_values.calorific_value = 10200;
...
function sendRegionValues(){
	...
	$.ajax({
		type: "POST",
		url: "{{ url('calculators/double-effect-g2/ajax-calculate-region') }}",
...
function updateModelValues(input_type){
	...
	switch(input_type) {
		...
		updateEvaporatorOptions(model_values.evaporator_material_value,true);
    		...
    		validate = inputValidation(model_values.calorific_value,"positive_decimals","calorific_value_error");
    		break;
	...
	$.ajax({
		type: "POST",
		url: "{{ url('calculators/double-effect-g2/ajax-calculate') }}",
...
$("#double_steam_s2").submit(function(event) {
	...
	var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
		...
		$.ajax({
			type: "POST",
			url: "{{ url('calculators/double-effect-g2/submit-calculate') }}",
			data: { values : model_values,_token: CSRF_TOKEN},
			complete: function(){
				$("#ajax-loader").hide();
			},
...
$( "#reset" ).click(function() {
	...
	$.ajax({
		type: "POST",
		url: "{{ url('calculators/double-effect-g2/reset-calculate') }}",
...
$( "#show_report" ).click(function() {
	...
	if(name == '' || project == '' || phone == ''){
		...
		$.ajax({
			type: "POST",
			url: "{{ url('calculators/double-effect-g2/show-report') }}",
...
$( ".save_report" ).click(function() {
	...
	if(name == '' || project == '' || phone == ''){
		...
		$.ajax({
			type: "POST",
			url: "{{ url('calculators/double-effect-g2/save-report') }}",
